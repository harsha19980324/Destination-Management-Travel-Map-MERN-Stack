...
export default class CreateDestination extends Component {
    ...
    validation = () => {
        ...
        if(!this.state.url1Error){
            url1Error="province Required!"
        }

        if(!this.state.image1Error){
            image1Error="Description Required!"
        }  
        if(titleError | descriptionError | description1Error | urlError | url1Error | image1Error ){     this.setState({ titleError , descriptionError , description1Error , urlError , url1Error , image1Error });      
