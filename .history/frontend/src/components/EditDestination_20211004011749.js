...
export default class EditDestination extends Component {
    ...
    onSubmit = (e) =>{
        ...
        axios.put(`http://localhost:8000/destination/update/${id}`,data).then((res)=>{
            if(res.data.success){
    ...
    componentDidMount(){
        const id = this.props.match.params.id;
        axios.get(`http://localhost:8000/destination/${id}`).then((res)=>{
            if(res.data.success){
                this.setState({
    ...
    onChangeImage1=event=>{
        this.setState({
            ...
            axios.post("http://localhost:8000/destination/upload", data, { 
            }).then(res => { 
